...
export function BrandingProvider({ children }) {
  ...
  const isPending =
    !user || user.status === 'pending' || (!user.hotel_id && user.role !== 'SUPER_ADMIN')
  ...
  useEffect(() => {
    // Pending проверяем по status и hotel_id
    const isPendingUser =
      !user || user.status === 'pending' || (!user.hotel_id && user.role !== 'SUPER_ADMIN')
    if (isPendingUser) {
      setLoading(false)
      // Apply defaults for pending users
  ...
  const resetBranding = useCallback(async () => {
    try {
      ...
      if (response.success) {
        // Use returned branding from server instead of hardcoded defaults
        const newBranding = response.branding || DEFAULT_BRANDING
        setBranding(newBranding)
        applyBrandingToCSS(newBranding)
        return { success: true, branding: newBranding, loginBranding: response.loginBranding }
      }
