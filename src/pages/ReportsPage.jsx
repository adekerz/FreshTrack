...
import {
  HeartPulse, CalendarClock, ClipboardList, TrendingUp,
  BarChart3, PieChart, Layers, LayoutDashboard,
...
const STATUS_LABELS = {
  good: 'Хорошо', warning: 'Внимание', critical: 'Критично',
  today: 'Сегодня', expired: 'Просрочено'
}
...
function Delta({ value }) {
  ...
  if (v === 0) return <span className="flex items-center gap-0.5 text-xs text-muted-foreground"><Minus className="w-3 h-3" />0</span>
  const pos = v > 0
  return (
...
function ErrorState({ message, onRetry }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      {onRetry && (
        <button onClick={onRetry} className="text-xs text-accent underline">Повторить</button>
      )}
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted-foreground">Текущее состояние инвентаря по каждому отделу</p>
        <div className="flex items-center gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.healthSummary} title="Здоровье инвентаря" filename="health_summary" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
      ...
      {!loading && !error && rows.length === 0 && <EmptyState message="Нет данных об инвентаре" />}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                ...
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Всего</th>
                <th className="text-center px-3 py-3 font-medium text-emerald-600">Хорошие</th>
                <th className="text-center px-3 py-3 font-medium text-amber-600">Внимание</th>
                <th className="text-center px-3 py-3 font-medium text-orange-600">Критично</th>
                <th className="text-center px-3 py-3 font-medium text-red-600">Просрочено</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Health Score</th>
              </tr>
            </thead>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <p className="text-sm text-muted-foreground">Товары с истекающим сроком в ближайшие</p>
        ...
        <div className="flex items-center gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.expiryForecast} title={`Прогноз истечения (${days} дн.)`} filename="expiry_forecast" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && rows.length === 0 && <EmptyState message="Нет товаров с истекающим сроком" />}
      ...
      {!loading && rows.length > 0 && (
        <div className="overflow-x-auto rounded-xl border border-border">
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Продукт</th>
                <th className="text-left px-3 py-3 font-medium text-muted-foreground">Отдел</th>
                <th className="text-left px-3 py-3 font-medium text-muted-foreground">Категория</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Дата истечения</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Дней</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Кол-во</th>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-end gap-3 flex-wrap">
        ...
        <div className="flex-1 flex items-center justify-end gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.collectionActivity} title="Активность сборов" filename="collection_activity" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && rows.length === 0 && <EmptyState message="Нет данных о сборах за выбранный период" />}
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="text-left px-3 py-3 font-medium text-muted-foreground">Продукт</th>
                  <th className="text-left px-3 py-3 font-medium text-muted-foreground">Отдел</th>
                  <th className="text-left px-3 py-3 font-medium text-muted-foreground">Кто собрал</th>
                  <th className="text-center px-3 py-3 font-medium text-muted-foreground">Кол-во</th>
                  <th className="text-left px-3 py-3 font-medium text-muted-foreground">Причина</th>
                  <th className="text-center px-3 py-3 font-medium text-muted-foreground">Срок годности</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {rows.map((row, i) => (
                  <tr key={i} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-3 py-3">
                      <span className="px-2 py-0.5 rounded-full text-xs bg-muted text-muted-foreground">{row.collection_reason || '—'}</span>
                    </td>
...
function DepartmentScorecardPanel() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted-foreground">Рейтинг отделов по качеству управления инвентарём</p>
        <div className="flex items-center gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.departmentScorecard} title="Рейтинг отделов" filename="department_scorecard" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && ranked.length === 0 && <EmptyState message="Нет данных по отделам" />}
              ...
              <div key={i} className="flex items-center gap-4 p-4 rounded-xl border border-border bg-card hover:shadow-sm transition-shadow">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-1 flex-wrap">
                    <span className="text-xs text-muted-foreground">{row.total_batches} партий</span>
                    <span className="text-xs text-red-500">{row.expired_count} просрочено ({row.expiry_rate_pct}%)</span>
                    <span className="text-xs text-muted-foreground">Ср. {row.avg_days_to_expiry} дн.</span>
                    <span className="text-xs text-emerald-600">{row.collections_this_month} сборов</span>
                  </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end gap-3 flex-wrap">
        ...
        <div className="flex-1 flex items-center justify-end gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.productTurnover} title="Оборот продуктов" filename="product_turnover" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && rows.length === 0 && <EmptyState message="Нет данных о продуктах" />}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">Продукт</th>
                <th className="text-left px-3 py-3 font-medium text-muted-foreground">Категория</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Партий</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Остаток</th>
                <th className="text-center px-3 py-3 font-medium text-emerald-600">Собрано</th>
                <th className="text-center px-3 py-3 font-medium text-red-500">Просрочено</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Доля потребл. %</th>
                <th className="text-center px-3 py-3 font-medium text-muted-foreground">Оборот</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {rows.map((row, i) => (
                <tr key={i} className="hover:bg-muted/30 transition-colors">
                  ...
                  <td className="text-center px-3 py-3">
                    {row.consumption_rate_pct != null
                      ? <span className={Number(row.consumption_rate_pct) >= 80 ? 'text-emerald-600 font-medium' : ''}>{row.consumption_rate_pct}%</span>
                      : '—'}
                  </td>
                  <td className="text-center px-3 py-3">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end gap-3 flex-wrap">
        ...
        <div className="flex-1 flex items-center justify-end gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.collectionReasons} title="Причины сборов" filename="collection_reasons" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && rows.length === 0 && <EmptyState message="Нет данных о причинах" />}

      {!loading && rows.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Bar Chart */}
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="text-sm font-medium text-foreground mb-4">По количеству</h3>
            <div className="space-y-3">
              {rows.map((row, i) => (
                <div key={i} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-foreground font-medium">{row.reason || 'Не указано'}</span>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="text-center px-3 py-3 font-medium text-muted-foreground">Транз.</th>
                  <th className="text-center px-3 py-3 font-medium text-muted-foreground">Кол-во</th>
                  <th className="text-center px-3 py-3 font-medium text-muted-foreground">Доля</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {rows.map((row, i) => (
                  <tr key={i} className="hover:bg-muted/30 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-foreground">{row.reason || 'Не указано'}</span>
                      </div>
                    </td>
...
function BatchAgePanel() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted-foreground">Распределение активных партий по остатку срока годности</p>
        <div className="flex items-center gap-2">
          <ExportToolbar data={rows} columns={EXPORT_COLUMNS.batchAgeDistribution} title="Распределение по возрасту" filename="batch_age_distribution" />
          <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && rows.length === 0 && <EmptyState message="Нет активных партий" />}

      {!loading && rows.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Visual bars */}
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="text-sm font-medium text-foreground mb-4">По количеству партий</h3>
            <div className="space-y-4">
              {rows.map((row, i) => {
                ...
                return (
                  <div key={i} className="space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-medium" style={{ color }}>{row.age_range}</span>
                      <span className="text-muted-foreground">{row.batch_count} партий · {row.percentage}%</span>
                    </div>
                    <div className="h-3 rounded-full bg-muted overflow-hidden">
                      <div className="h-full rounded-full transition-all duration-700"
                        style={{ width: `${(Number(row.batch_count) / maxCount) * 100}%`, backgroundColor: color }} />
                    </div>
                    <p className="text-xs text-muted-foreground">{row.total_quantity} ед. общего количества</p>
                  </div>
                )
              })}
            </div>
            ...
            {rows.map((row, i) => {
              ...
              return (
                <div key={i} className="bg-card border border-border rounded-xl p-4 space-y-1"
                  ...
                  <p className="text-xs font-medium" style={{ color }}>{row.age_range}</p>
                  <p className="text-2xl font-bold text-foreground">{row.batch_count}</p>
                  <p className="text-xs text-muted-foreground">{row.percentage}% · {row.total_quantity} ед.</p>
                </div>
              )
            })}
          </div>
...
function WeeklySummaryPanel() {
  ...
  const summary = data && typeof data === 'object' && !Array.isArray(data) ? data : (Array.isArray(data) ? data[0] : null)

  const kpiCards = summary ? [
    { label: 'Всего партий', value: summary.total_batches, icon: Layers, color: 'text-foreground' },
    { label: 'Хорошие', value: summary.good, icon: CheckCircle, color: 'text-emerald-600' },
    { label: 'Просрочено', value: summary.expired, icon: XCircle, color: 'text-red-500' },
    { label: 'Health Score', value: summary.health_score, icon: HeartPulse, color: 'text-accent', delta: summary.health_delta, isScore: true },
    { label: 'Сборов за неделю', value: summary.collections, icon: ClipboardList, color: 'text-blue-500' },
    { label: 'Собрано единиц', value: summary.collected_qty, icon: TrendingUp, color: 'text-emerald-600' }
  ] : []

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted-foreground">Сводка ключевых KPI за последние 7 дней</p>
        <button onClick={reload} disabled={loading} className="p-1.5 rounded-lg border border-border hover:bg-muted transition-colors disabled:opacity-50">
          <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
        </button>
      </div>
      ...
      {loading && <div className="flex justify-center py-12"><Loader /></div>}
      {error && <ErrorState message={error} onRetry={reload} />}
      {!loading && !error && !summary && <EmptyState message="Нет данных за неделю" />}
          ...
          {summary.prev_health_score != null && (
            <div className="bg-muted/30 rounded-xl p-4 border border-border">
              <p className="text-sm text-muted-foreground">
                Health Score изменился на <strong><Delta value={summary.health_delta} /></strong> по сравнению с прошлой неделей
                (было: {summary.prev_health_score}, стало: {summary.health_score})
              </p>
            </div>
          )}
        </>
...
const TABS = [
  { id: 'health', label: 'Здоровье', icon: HeartPulse, panel: HealthSummaryPanel },
  { id: 'expiry', label: 'Прогноз истечения', icon: CalendarClock, panel: ExpiryForecastPanel },
  { id: 'collections', label: 'Сборы', icon: ClipboardList, panel: CollectionActivityPanel },
  { id: 'departments', label: 'Рейтинг отделов', icon: BarChart3, panel: DepartmentScorecardPanel },
  { id: 'turnover', label: 'Оборот', icon: TrendingUp, panel: ProductTurnoverPanel },
  { id: 'reasons', label: 'Причины', icon: PieChart, panel: CollectionReasonsPanel },
  { id: 'age', label: 'Возраст партий', icon: Layers, panel: BatchAgePanel },
  { id: 'weekly', label: 'Неделя', icon: LayoutDashboard, panel: WeeklySummaryPanel }
]
    ...
    <AnimatedPage>
      <PageContainer>
        <div className="space-y-6">
          {/* Header */}
          <div>
            <h1 className="text-2xl font-bold text-foreground">Аналитические отчёты</h1>
