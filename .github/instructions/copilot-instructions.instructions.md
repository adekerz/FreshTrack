...
Перед написанием кода **ВСЕГДА задай себе вопросы**:

### 1. Нужно ли это вообще?
- Решает ли это реальную проблему пользователя?
- Не дублирует ли существующий функционал?
- Попросили бы это 80% пользователей?

### 2. Не нарушает ли концепцию?
- Backend остаётся единственным источником истины?
- Permissions проверяются на бэке, не на фронте?
- Данные изолированы по hotel/department?

### 3. UX/UI проверка
- Понятно ли это с первого взгляда?
- Сколько кликов нужно для действия? (цель: ≤3)
- Работает ли на мобильном? (touch targets ≥48px)
- Есть ли feedback на действия? (loading, success, error)
...
|------|----------|------------------|
| **Touch targets** | https://m3.material.io/foundations/accessible-design | Минимум 48×48px |
| **Формы** | https://www.nngroup.com/articles/web-form-design/ | Labels сверху, не сбоку |
| **Таблицы mobile** | https://css-tricks.com/responsive-data-tables/ | Карточки вместо таблиц |
| **Навигация** | https://www.nngroup.com/articles/mobile-navigation/ | Bottom tabs > hamburger |
| **Feedback** | https://www.nngroup.com/articles/response-times-3-important-limits/ | <100ms = мгновенно, <1s = без loader, >1s = показать loader |
| **Цвета** | https://webaim.org/resources/contrastchecker/ | Контраст ≥4.5:1 для текста |
| **Cognitive load** | https://www.nngroup.com/articles/minimize-cognitive-load/ | Не более 7±2 элементов |

---

## ✅ UX ЧЕКЛИСТ (проверяй каждый компонент)

### Понятность
- [ ] Назначение элемента ясно без объяснений?
- [ ] Текст кнопок = действие (не "ОК", а "Сохранить", "Удалить")
- [ ] Иконки имеют подписи или tooltip?

### Удобство
- [ ] Главное действие — самое заметное (primary button)
- [ ] Деструктивные действия требуют подтверждения
- [ ] Можно отменить действие или вернуться назад?
- [ ] Форма сохраняет данные при случайном уходе?

### Feedback
- [ ] Loading state при ожидании >1 сек
- [ ] Success/Error toast после действия
- [ ] Disabled состояние при невозможности действия
- [ ] Empty state когда данных нет

### Mobile
- [ ] Кнопки ≥48px для touch
- [ ] Нет hover-only функционала
- [ ] Читаемо без зума (font ≥16px)
- [ ] Работает в landscape и portrait

### Accessibility
- [ ] Контраст текста ≥4.5:1
- [ ] Фокус виден при Tab навигации
- [ ] aria-label на иконках-кнопках
- [ ] Ошибки связаны с полями (aria-describedby)

---

## 🚫 UX АНТИПАТТЕРНЫ (никогда не делай)

| ❌ Плохо | ✅ Хорошо |
|----------|-----------|
| Модалка в модалке | Bottom sheet или отдельная страница |
| Удаление без подтверждения | Confirm dialog с названием объекта |
| "Произошла ошибка" | "Не удалось сохранить: сервер недоступен" |
| Бесконечный spinner | Timeout + кнопка "Повторить" |
| Скрытая навигация (hamburger) | Видимые tabs (≤5 пунктов) |
| Красный текст без иконки | Иконка ⚠️ + красный текст |
| Форма без валидации | Inline validation на blur |
| Таблица 10+ колонок на mobile | Card layout или приоритетные колонки |
...
Если данных нет — UI должен явно показать ошибку, не падать молча.
...
**Тест:** Новая роль = **0 изменений кода**.

---

## 🏨 ИЗОЛЯЦИЯ ДАННЫХ (КРИТИЧНО)

Каждый backend-запрос ОБЯЗАН:
- фильтр по `hotel_id`
- фильтр по `department_id` (если применимо)
- использовать `buildContextWhere(req.user)`

❌ Запросы без контекста пользователя ЗАПРЕЩЕНЫ.

---

## ⚙️ BACKEND ПРАВИЛА
```
Route → requirePermission → Service → Audit → Response
```

- Роуты тонкие, вся логика в `services/`
- Каждый роут → `requirePermission()`
- Каждая мутация → `logAudit()` с snapshot
- ❌ fallback permissions
- ❌ публичные admin/health эндпоинты без auth
...
// ❌ НИКОГДА
if (user.role === 'ADMIN') { }
const isExpired = daysLeft <= 0  // логика!
const color = status === 'critical' ? 'red' : 'green'  // hardcode!
```

### Правильно:
```javascript
// ✅ ВСЕГДА — данные с бэка
...
Backend возвращает:
- `status` — enum статус
- `statusColor` — CSS класс/цвет
- `statusLabel` — локализованный текст
- `permissions` — что можно делать с объектом

---

## ⚡ REALTIME (SSE)

Настройки (брендинг, пороги, тексты) могут меняться в realtime.

UI должен:
- Подписаться на SSE события
- Обновлять Context при получении события
...
1. **Явно укажи допущения** в комментарии
...
| Запрет | Почему |
|--------|--------|
| Hardcoded роли | Нарушает RBAC систему |
| Бизнес-логика в UI | Backend = source of truth |
| Permission fallback | Безопасность |
| Cross-hotel доступ | Изоляция данных |
| console.log в проде | Безопасность + производительность |
| any в TypeScript | Типобезопасность |
| Inline styles | Консистентность дизайна |
| Magic numbers | Читаемость кода |
...
1. **Объясни решение** (1-2 предложения)
...
useEffect(() => {
  ...
  eventSource.addEventListener('branding-update', (e) => {
    ...
    setBranding(data.settings) // Обновляем context
  })
...
// Проверить:
// - [ ] SSE endpoint существует
// - [ ] Auth token передаётся
