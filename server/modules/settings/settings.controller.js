  ...
  try {
    ...
    res.json({
      ...
      thresholds: {
        ...
        attention: thresholds.attention ?? 5,
  ...
  try {
    ...
    const branding = {
      ...
      app_name:
        ...
        allSettings.branding?.siteName ??
        raw.app_name ??
        BRANDING_DEFAULTS.app_name,
      app_tagline: raw.app_tagline ?? BRANDING_DEFAULTS.app_tagline,
      company_name:
        raw['branding.companyName'] ??
        allSettings.branding?.companyName ??
        raw.company_name ??
        BRANDING_DEFAULTS.company_name,
      dashboard_title: raw.dashboard_title ?? BRANDING_DEFAULTS.dashboard_title,
      welcome_message:
        raw['branding.welcomeMessage'] ??
        allSettings.branding?.welcomeMessage ??
        raw.welcome_message ??
        BRANDING_DEFAULTS.welcome_message,
      logo_url:
        raw['branding.logoUrl'] ??
        allSettings.branding?.logoUrl ??
        raw.logo_url ??
        BRANDING_DEFAULTS.logo_url,
      logo_dark_url:
        raw['branding.logoDark'] ??
        allSettings.branding?.logoDark ??
        raw.logo_dark_url ??
        BRANDING_DEFAULTS.logo_dark_url,
      favicon_url:
        raw['branding.faviconUrl'] ??
        allSettings.branding?.faviconUrl ??
        raw.favicon_url ??
        BRANDING_DEFAULTS.favicon_url,
      primary_color:
        raw['branding.primaryColor'] ??
        allSettings.branding?.primaryColor ??
        raw.primary_color ??
        BRANDING_DEFAULTS.primary_color,
      secondary_color:
        raw['branding.secondaryColor'] ??
        allSettings.branding?.secondaryColor ??
        raw.secondary_color ??
        BRANDING_DEFAULTS.secondary_color,
      accent_color:
        raw['branding.accentColor'] ??
        allSettings.branding?.accentColor ??
        raw.accent_color ??
        BRANDING_DEFAULTS.accent_color,
      danger_color:
        raw['branding.dangerColor'] ??
        allSettings.branding?.dangerColor ??
        raw.danger_color ??
        BRANDING_DEFAULTS.danger_color,
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get branding settings' })
  }
})
  ...
  async (req, res) => {
    try {
      ...
      if (!branding || typeof branding !== 'object') {
        return res
          .status(400)
          .json({ success: false, error: 'Branding object is required' })
      }
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to update branding settings' })
    }
  }
)
    ...
    try {
      ...
      await logAudit({
        ...
        details: {
          ...
          includedSystemLevel: isSuperAdmin,
        },
        ip_address: req.ip,
      })
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to reset branding settings' })
    }
  }
)
...
router.get('/general', authMiddleware, hotelIsolation, async (req, res) => {
  try {
    ...
    for (const key of GENERAL_KEYS) {
      settings[key] =
        allSettings.general?.[key] ?? allSettings[key] ?? GENERAL_DEFAULTS[key]
    }
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get general settings' })
  }
})
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to update general settings' })
    }
  }
)
...
router.get('/telegram', authMiddleware, hotelIsolation, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get telegram settings' })
  }
})
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      logInfo(
        'PUT /telegram',
        `Received update: sendTime=${sendTime}, timezone=${timezone}, templatesUpdated=${!!messageTemplates}`
      )
      ...
      if (sendTime) {
        ...
        try {
          const { rescheduleDailyReport } =
            await import('../../jobs/notificationJobs.js')
      ...
      res.json({
        success: true,
        message: sendTime
          ? `Notifications rescheduled to ${sendTime}`
          : 'Settings saved',
      })
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    ...
    `,
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get telegram chats' })
    }
  }
)
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const { is_active, notify_critical, notify_warning, notify_expired } =
        req.body
    ...
    `,
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to update telegram chat' })
    }
  }
)
...
router.delete(
  ...
  async (req, res) => {
    ...
    `,
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to unlink telegram chat' })
    }
  }
)
  ...
  async (req, res) => {
    try {
      logInfo(
        ...
        '🧪 Manual test notification triggered'
      )
      ...
      try {
        ...
        logInfo(
          'POST /test-notification',
          `📊 Daily report: ${results.sent} sent (Telegram: ${results.telegram}, Email: ${results.email})`
        )
      } catch (err) {
...
router.get('/login-branding', async (req, res) => {
  try {
    ...
    const loginBranding = {
      title:
        systemSettings['branding.login.title'] ?? LOGIN_BRANDING_DEFAULTS.title,
      highlight:
        systemSettings['branding.login.highlight'] ??
        LOGIN_BRANDING_DEFAULTS.highlight,
      description:
        systemSettings['branding.login.description'] ??
        LOGIN_BRANDING_DEFAULTS.description,
      feature1:
        systemSettings['branding.login.feature1'] ??
        LOGIN_BRANDING_DEFAULTS.feature1,
      feature2:
        systemSettings['branding.login.feature2'] ??
        LOGIN_BRANDING_DEFAULTS.feature2,
      feature3:
        systemSettings['branding.login.feature3'] ??
        LOGIN_BRANDING_DEFAULTS.feature3,
      // Also include site name and colors for login page styling
      siteName:
        systemSettings['branding.siteName'] ?? BRANDING_DEFAULTS.app_name,
      primaryColor:
        systemSettings['branding.primaryColor'] ??
        BRANDING_DEFAULTS.primary_color,
    }
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get login branding' })
  }
})
...
router.put('/login-branding', authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== 'SUPER_ADMIN') {
      return res
        .status(403)
        .json({ success: false, error: 'SUPER_ADMIN required' })
    }
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to update login branding' })
  }
})
...
router.get('/user', authMiddleware, hotelIsolation, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get user settings' })
  }
})
  ...
  try {
    ...
    if (result.success) {
      await logAudit({
        ...
        snapshot_before:
          result.before !== null ? { [key]: result.before } : null,
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to set user setting' })
  }
})
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to delete user setting' })
    }
  }
)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get department settings' })
    }
  }
)
  ...
  async (req, res) => {
    try {
      ...
      if (!req.departmentId) {
        return res
          .status(400)
          .json({ success: false, error: 'Department context required' })
      }
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to set department setting' })
    }
  }
)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get hotel settings' })
    }
  }
)
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to set hotel setting' })
    }
  }
)
...
router.get('/system', authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get system settings' })
  }
})
...
router.put('/system/:key', authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to set system setting' })
  }
})
    ...
    try {
      ...
      if (scope === 'system' && req.user.role !== 'SUPER_ADMIN') {
        return res
          ...
          .json({
            success: false,
            error: 'SUPER_ADMIN required for system scope',
          })
      }
      ...
      if (
        scope === 'hotel' &&
        !['HOTEL_ADMIN', 'SUPER_ADMIN'].includes(req.user.role)
      ) {
        return res
          .status(403)
          .json({
            success: false,
            error: 'HOTEL_ADMIN required for hotel scope',
          })
      }
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to batch update settings' })
    }
  }
)
  ...
  async (req, res) => {
    ...
    `,
      ...
      if (result.rows.length > 0 && result.rows[0].value) {
        const parsed =
          typeof result.rows[0].value === 'string'
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get notification rules' })
    }
  }
)
...
router.put(
  ...
  async (req, res) => {
    ...
    `,
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to update notification rules' })
    }
  }
)
  ...
  async (req, res) => {
    try {
      ...
      const telegramEnabled =
        allSettings.raw?.['notify.channels.telegram'] ??
        allSettings.notify?.channels?.telegram ??
        true // Default to true if telegram settings exist
      const emailEnabled =
        allSettings.raw?.['notify.channels.email'] ??
        allSettings.notify?.channels?.email ??
        false
      ...
      // Get timezone
      const timezone =
        allSettings.raw?.['locale.timezone'] ??
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get notifications settings' })
    }
  }
)
    ...
    try {
      ...
      if (sendTime) {
        ...
        try {
          ...
          logInfo(
            'PUT /notifications',
            `✅ Daily report rescheduled to ${sendTime}`
          )
        } catch (jobError) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const settings = {
        enabled:
          allSettings.raw?.['email.enabled'] ??
          allSettings.email?.enabled ??
          false,
        smtpHost:
          allSettings.raw?.['email.smtpHost'] ??
          allSettings.email?.smtpHost ??
          process.env.SMTP_HOST ??
          '',
        smtpPort:
          allSettings.raw?.['email.smtpPort'] ??
          allSettings.email?.smtpPort ??
          parseInt(process.env.SMTP_PORT || '587'),
        smtpSecure:
          allSettings.raw?.['email.smtpSecure'] ??
          allSettings.email?.smtpSecure ??
          process.env.SMTP_SECURE === 'true',
        smtpUser:
          allSettings.raw?.['email.smtpUser'] ??
          allSettings.email?.smtpUser ??
          process.env.SMTP_USER ??
          '',
        smtpPassword:
          allSettings.raw?.['email.smtpPassword'] ??
          allSettings.email?.smtpPassword ??
          process.env.SMTP_PASS ??
          '',
        fromEmail:
          allSettings.raw?.['email.fromEmail'] ??
          allSettings.email?.fromEmail ??
          process.env.EMAIL_FROM ??
          'no-reply@freshtrack.systems',
        fromName:
          allSettings.raw?.['email.fromName'] ??
          allSettings.email?.fromName ??
          'FreshTrack',
        dailyReportTime:
          allSettings.raw?.['email.dailyReportTime'] ??
          allSettings.email?.dailyReportTime ??
          '08:00',
        dailyReportEnabled:
          allSettings.raw?.['email.dailyReportEnabled'] ??
          allSettings.email?.dailyReportEnabled ??
          true,
        instantAlertsEnabled:
          allSettings.raw?.['email.instantAlertsEnabled'] ??
          allSettings.email?.instantAlertsEnabled ??
          true,
      }
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get email settings' })
    }
  }
)
    ...
    try {
      ...
      if (settings.dailyReportTime) {
        try {
          ...
          logInfo(
            'PUT /email',
            `✅ Daily report rescheduled to ${settings.dailyReportTime}`
          )
        } catch (jobError) {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to update email settings' })
    }
  }
)
    ...
    `,
      ...
      if (deptResult.rows.length === 0) {
        return res.status(400).json({
          success: false,
          error:
            'No department with email found. Please set email for at least one department first.',
        })
      }
      ...
      await sendEmail({
        ...
        from: settings?.fromEmail
          ? `${settings.fromName || 'FreshTrack'} <${settings.fromEmail}>`
          : EMAIL_FROM.noreply,
