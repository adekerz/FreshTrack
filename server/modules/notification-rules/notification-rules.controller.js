...
import {
  ...
  TestTelegramSchema,
} from './notification-rules.schemas.js'
...
function requireAdminRole(req, res, next) {
  ...
  if (req.user.role !== 'SUPER_ADMIN' && req.user.role !== 'HOTEL_ADMIN') {
    return res.status(403).json({
      success: false,
      error:
        'Access denied. Only SUPER_ADMIN and HOTEL_ADMIN can manage notification rules.',
    })
  }
    ...
    try {
      ...
      const hotelId =
        req.user.role === 'SUPER_ADMIN'
          ? req.query.hotel_id || req.hotelId
          : req.hotelId
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to create notification rule' })
    }
  }
)
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to toggle notification rule' })
    }
  }
)
  ...
  async (req, res) => {
    try {
      ...
      if (rule.rows[0].hotel_id === null) {
        return res
          .status(403)
          .json({ success: false, error: 'Cannot delete system rule' })
      }
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to delete notification rule' })
    }
  }
)
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to delete telegram chat' })
    }
  }
)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to run expiry check' })
    }
  }
)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to run queue process' })
    }
  }
)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, error: 'Failed to get notification stats' })
    }
  }
)
