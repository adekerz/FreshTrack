...
router.get('/stream', authMiddleware, hotelIsolation, (req, res) => {
  try {
    ...
    const keepAliveInfo = setInterval(() => {
      res.write(': keep-alive\n\n')
    }, 15000)

    // Handle client disconnect
    req.on('close', () => {
      clearInterval(keepAliveInfo)
    ...
    // If headers already sent, we can't send 500
    if (!res.headersSent) {
...
router.post(
  ...
  authMiddleware,
  hotelIsolation,
  requirePermission('events', 'broadcast'),
  (req, res) => {
    ...
    logInfo(
      'SSE',
      `Admin broadcast [${event}] by ${req.user.name}: ${sent} clients`
    )
...
router.post(
  ...
  (req, res) => {
    ...
    if (!userId || !event || !data) {
      return res
        .status(400)
        .json({ error: 'userId, event, and data are required' })
    }
