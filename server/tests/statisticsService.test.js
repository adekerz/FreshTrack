...
describe('StatisticsService - Phase 3: Centralized Statistics', () => {
  ...
  describe('getStatistics() - Context Filtering', () => {
    it('should require hotelId', async () => {
      await expect(StatisticsService.getStatistics({})).rejects.toThrow(
        'hotelId required for statistics'
      )
    })
  ...
  describe('byStatus - Expiry Status Breakdown', () => {
    it('should return all status categories with colors from backend', async () => {
      ...
      const expiredStatus = result.byStatus.find(
        (s) => s.status === ExpiryStatus.EXPIRED
      )
    ...
    it('should calculate percentages correctly', async () => {
      ...
      const warningStatus = result.byStatus.find(
        (s) => s.status === ExpiryStatus.WARNING
      )
    ...
    it('should include localized labels', async () => {
      ...
      const resultRu = await StatisticsService.getStatistics(
        { hotelId: mockHotelId },
        { locale: 'ru' }
      )
      expect(
        resultRu.byStatus.find((s) => s.status === ExpiryStatus.GOOD).label
      ).toBe('В норме')

      const resultEn = await StatisticsService.getStatistics(
        { hotelId: mockHotelId },
        { locale: 'en' }
      )
      expect(
        resultEn.byStatus.find((s) => s.status === ExpiryStatus.GOOD).label
      ).toBe('Good')
    })
  })
  ...
  describe('byCategory - No "Other" Category', () => {
    it('should group by category_id from batch (not product lookup)', async () => {
      ...
      // No "Other" category
      expect(
        result.byCategory.find((c) => c.categoryName === 'Другое')
      ).toBeUndefined()
      expect(
        result.byCategory.find((c) => c.categoryName === 'Other')
      ).toBeUndefined()
    })
    ...
    it('should include category color from database', async () => {
      ...
      const dairyCategory = result.byCategory.find(
      ...
      const mockBatches = [
        {
          ...
          expiryStatus: ExpiryStatus.TODAY,
          quantity: 10,
          category_id: 'cat-1',
        },
        {
  ...
  describe('getQuickStats() - Dashboard Widget', () => {
    it('should return summary stats', async () => {
      ...
      const result = await StatisticsService.getQuickStats({
        hotelId: mockHotelId,
      })
