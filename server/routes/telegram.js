/**
 * FreshTrack Telegram API Routes
...
router.post('/chats', authMiddleware, requirePermission('settings', 'write'), async (req, res) => {
  try {
    ...
    logInfo('TelegramRoutes', `Chat ${chatId} linked to hotel ${hotelId}`)
    ...
    res.json({
      ...
      instructions: {
        ru: [
          ...
          '3. Отправьте в чат команду: /link hotel:RC-ASTANA',
          '4. Для привязки к конкретному отделу: /link hotel:RC-ASTANA department:HONOR_BAR',
          '5. Проверьте статус: /status'
        ],
        commands: {
          '/link hotel:КОД': 'Привязать чат к отелю',
